...
var Fuerza = (function () {
  ...
  // Qué ejercicios del Excel cuentan como cada levantamiento de la web (nombre sin acentos ni mayúsculas).
  // Las variantes en máquina, Smith o con mancuernas no entran: la web solo tiene datos de estos ejercicios.
  var EQUIVALENCIAS = [
    ...
    ['powerClean', /^(cargada|power clean)/],
    ['benchPress', /^(press (de )?banca|press supino|supine press|press plano con barra)( con barra)?$/],
    ['inclineBenchPress', /^press (de )?(banca )?inclinado( con barra| \(barra\))?$/],
    ['dip', /^fondos( en paralelas| lastrados| triceps)?$/],
    ['overheadPress', /^pres{1,2} militar( barra| con barra| de pie)?$/],
    ['pushPress', /^push press/],
    ['chinup', /^dominadas( lastradas| supinas| asistidas)?$/],
    ['pendlayRow', /^remo (en barra|con barra|pendl?e?y|en bar{1,2}a \(?smith\)?)/],
  ];
  ...
  function levantamientoDe(nombre) {
    var clave = normalizar(nombre);
    for (var i = 0; i < EQUIVALENCIAS.length; i++) if (EQUIVALENCIAS[i][1].test(clave)) return EQUIVALENCIAS[i][0];
    return null;
  }
  ...
  function mejores(ejercicios, persona, hasta, dias) {
    ...
    ejercicios.forEach(function (e) {
      var id = levantamientoDe(e.nombre);
      if (!id) return;
      ...
      e.dias.forEach(function (d) {
        ...
        d.s.forEach(function (s) {
          ...
          if (!rm || rm <= 0) return;
          if (!mejor[id] || rm > mejor[id].rm) mejor[id] = { rm: rm, kg: Number(s[0]) || 0, reps: Number(s[1]), f: d.f, ejercicio: e.nombre };
        });
      });
    });
    return mejor;
